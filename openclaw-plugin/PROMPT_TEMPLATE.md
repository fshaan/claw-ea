...
9. **如果有日程/任务**：先展示摘要让用户确认，确认后调用对应工具：
   - **手术安排**：仅调用 `claw_create_reminder`（不建笔记、不建日历事件）。
     只为 config 中 `user.name/aliases` 在 主刀/带组/一助 角色出现的台次建提醒。
     **台次时间推算**：台次格式 `{房间号}-{台次数}`（如 502-2 → 台次 2）。
     台次数对应 `due_date`：1→09:00, 2→13:00, 3→17:00, 4→20:00（当天日期 + 时间）。
     `due_date` 格式：ISO-8601，如 `2026-05-07T09:00:00`。
